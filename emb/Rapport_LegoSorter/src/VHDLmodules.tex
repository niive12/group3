...
\centering
 ...
 \draw[->] (stateR)   edge[bend left=27] node[midway,fill=white]                                  {sample\_done}            (stateG);
 \draw[->] (stateG)   edge[bend left=27] node[midway,below]                                       {sample\_done}            (stateB);
 \draw[->] (stateB)   edge[bend left=27] node[midway,fill=white]                                  {sample\_done}            (decide);
...
\subsubsection{Implementation}
Given the datasheet \footnote{Add reference to datasheet}
\nikolaj{reference mangler side nummer}
...
\nikolaj{Er det ikke fordi det tager for lang tid at synthesize og flashe fpga'en hver gang man vil ændre en værdi?
Ellers skal der testes om lys i rummet påvirker systemet.
}