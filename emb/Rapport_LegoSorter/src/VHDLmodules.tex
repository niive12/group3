...
\subsubsection{Test of Decision state.}
To find the ideal initial values for the blocks, a test with a each color block and a test without any blocks were 
...
To make the communication on the computer a small C++ program was made, using the Boost library to instantiate the serial communication, building on a skeleton found on the internet \footnote{\url{http://www.webalice.it/fede.tft/serial_port/serial_port.html}}.
...
\matthias{Der er ingen kontrol bits lige nu på modtag siden}
...
\nikolaj{Is this actually done or should we do this tomorrow?}
